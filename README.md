...
## 工夫した点・苦労した点
- **検索精度の改善**: ベクトル検索単体では類似度スコアの閾値（`score_threshold`）を調整しても専門用語の検索漏れが解消しなかったため、BM25によるキーワード検索を組み合わせたハイブリッド検索を実装しました。
- **ストリーミング応答のパース**: Ollamaの応答は「1行に1つのJSON」という形式で少しずつ送られてきますが、ネットワーク通信で一度に受信できるデータの区切りは、このJSON1件分の区切りと必ずしも一致しません。そのため、1つのJSONオブジェクトが2回に分けて届き、片方だけでは中身が不完全な文字列になってパースエラーになることがありました。そこで、受信したデータを一旦バッファに貯めておき、改行文字が現れた時点でそこまでの内容を「1件分のJSON」として区切って読み込む方式に変更し、この問題を解消しました。
- **商品レコメンドの出し分け**: 商品情報とナレッジ情報を同じsystem promptに含めると、ユーザーが求めていない場面でもLLMが商品を紹介してしまう問題がありました。ユーザーの発言に「おすすめ」といったキーワードが含まれる場合のみ、レスポンスに商品情報を含める形にしました。
